  ...
  <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
    <thead class="bg-gray-50 dark:bg-neutral-800">
      <tr>
        ...
        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Shift</th>
        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Dokter</th>
        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase dark:text-neutral-500">Ruang & Poli</th>
    ...
    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
      @forelse($jadwals as $index => $j)
        <tr>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200">{{ $jadwals->firstItem() + $index }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
            <span class="font-bold text-blue-600 dark:text-blue-500">{{ $j->shift->nama ?? '-' }}</span>
            <div class="text-xs text-gray-500">{{ $j->shift->hari ?? '' }} ({{ $j->shift->jam_masuk ?? '' }} - {{ $j->shift->jam_keluar ?? '' }})</div>
          </td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $j->dokter->nama ?? '-' }}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">
          ...
          <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
            ...
            <button type="button" 
                    ...
                    onclick="CrudHandler.confirmDelete('{{ route('jadwal.destroy', $j->id) }}', 'table-container')">
              Delete
            </button>
          </td>
        </tr>
      @empty
        <tr>
          <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500 dark:text-neutral-500">Jadwal tidak ditemukan.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
