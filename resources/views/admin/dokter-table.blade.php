    ...
    <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
      @forelse($dokters as $index => $d)
        ...
        <tr>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-neutral-200">{{ $u->username ?? '-' }}
          </td>
              ...
              <form id="edit-dokter-form-{{ $d->id }}" action="{{ route('dokter.update', $d->id) }}" method="POST">
                ...
                <div class="space-y-4 text-start">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label class="block text-sm font-medium mb-2 dark:text-white">Poli</label>
                      <select name="id_poli" disabled
                        class="disabled:opacity-100 py-2 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 pointer-events-none bg-gray-100 dark:bg-neutral-800">
                        @foreach($polis as $p)
                          <option value="{{ $p->id }}" {{ $d->id_poli == $p->id ? 'selected' : '' }}>{{ $p->nama_poli }}
                          </option>
                        @endforeach
                      </select>
                    </div>
                  ...
                  <div>
                    <label class="block text-sm font-medium mb-2 dark:text-white">Password (Biarkan kosong jika
                      tetap)</label>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input type="text" name="no_ktp" value="{{ $d->no_ktp }}"
                        class="py-2 px-4 block w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400">
                    </div>
                  </div>
                  <div>
      ...
      @empty
        <tr>
          <td colspan="6" class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-500 dark:text-neutral-500">Data
            tidak ditemukan.</td>
        </tr>
      @endforelse
    </tbody>
